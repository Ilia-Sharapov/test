...
if (Configure::read('debug')) {
    Configure::write('Cache._cake_model_.duration', '+2 seconds');
...
if (!function_exists("array_map_keys")) {
    function array_map_keys($callable,$array) {
        if (!is_array($array) || empty($array) || !is_callable($callable)) {
            return NULL;
        }
...
/**
 * Get object properties via dot notation
 * @param $obj
 * @param string $path
 * @return mixed
 */
function object_get_property_path($obj,$path = '')
{
    return array_reduce(explode('.', $path), function ($o, $p) { return is_numeric($p) ? ($o[$p] ?? null) : ($o->$p ?? null); }, $obj);
}

/**
 * Check if method implemented exactly in that class
 * @param $className
 * @param $methodName
 * @return bool
...
function propertyExistsInClass($className,$propertyName) {
    if (!property_exists($className, $propertyName)) {
        return false;
    }

    if(is_object($className)) $className = get_class($className);
    return (new ReflectionClass($className))->getProperty($propertyName)->class == $className;
}